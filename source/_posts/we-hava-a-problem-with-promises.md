...
如果你知道答案,那么祝贺你,你已经是一个promise高手了,可以不用再继续阅读这篇文章了.
<!-- more -->

答案是在这篇文章的最后,但首先,我想探讨为什么promise如此棘手,为什么我们这些许多新手和专家都被他们绊倒了.我也将提供我自己的见解,让promise简单易理解.是的,我真的认为它不太难！

但首先,让我们挑战关于promise的一些常见设想.

# 为什么要promise
如果你阅读有关promise的文献,你会发现回调造成的厄运金字塔,使代码稳步地向屏幕右侧延伸.

promise确实解决了这个问题,但不仅仅是缩进.正如优秀的演讲["Redemption from Callback Hell"](http://youtu.be/hf1T_AONQJU)中解释的那样,回调真正的问题是它们剥夺了我们如`return`和`throw`这样的关键字.但是,我们的程序的整个流程就是基于副作用：一个函数偶然地调用另一个函数.

事实上,回调干了更糟糕的事：它剥夺了我们的堆栈.没有堆栈的情况下编写代码就像在没有刹车踏板的情况下驾驶一辆汽车一样.

promise的全部意义就是当我们处理异步时让我们回到丢失的语言层面: `return`,`throw`还有堆栈.但是你必须知道怎样正确的使用promise以利用他们.

# 新手的错误
有些人尝试以漫画的形式解释promise,或者以名词化的方式: "这是一个你可以作为异步值传递的东西"

我不认为这样的解释有帮助.对我而言,promise都是关于代码结构和流程的.所以,我认为最好是重温一些常见的错误,并向他们展示如何解决这些错误.我把这些称为新手错误,意味着, "你现在是一个新手,但你不久会成为专家"

# 新手问题1: promise的厄运金字塔
看看人们如何使用PouchDB基于promise的API,我看到很多不好的promise模式.最常见的糟糕做法是：
```javascript
remotedb.allDocs({
  include_docs: true,
  attachments: true
...
如果你认为这种错误只限于绝对的初学者,你会惊讶地发现我实际上是从黑莓官方开发者博客上面得到的代码.旧的回调习惯很难改掉.（对于这个开发者：选择你很抱歉,但你的例子是有启发性的.）
...
```
这就是所谓的*"composing promises"*.每个函数只有在前一个promise已经resolve时才会被调用,并且会被传入该promise的输出.
...
```
这个代码有什么问题?问题是第一个函数实际上是返回的`undefined`,这意味着第二个函数不会等待`db.remove()`执行完操作.事实上,它不等待任何事情,并可以在删除了任何数量的文档时被执行！

这是一个特别隐秘的错误,因为假设PouchDB快速移除这些文档以使UI更新,你不会注意到它.这个bug只会在某些资源争夺的情况下出现,那几乎不可能进行调试.
...
```
这里发生了什么？基本上`Promise.all()`需要一个promise数组作为输入,然后它返回给你一个新的promise,只有当promise数组中的每一个都已经resolve时新的promise才能resolve.它是for循环的异步等价物.

`Promise.all()`还将结果数组传递给下一个函数,这可能会非常有用,例如,如果您尝试`get()`从PouchDB 执行多个任务.如果子promise中的任何一个被reject,在`all()`promise也会reject,这是更加有用.

# 新手错误3: 忘记添加`.catch()`
这是另一个常见的错误.他们自信自己的promise永远不会抛出一个错误,许多开发人员忘记在他们的代码中添加`.catch()`.不幸的是,这意味着任何抛出的错误都会被吞噬,而且甚至在控制台中都看不到.这才是真正的调试痛苦.
...
首先,大多数promise库都为您提供了从第三方库"import" Promise的方法.例如,Angular的$q模块允许你使用$q.when()来包装非$q-promise.所以Angular用户可以这样包装PouchDB的promise：
```javascript
...
```
只要记住: 对于一个被吞没,几乎不可能调试的错误来说,可以同步抛出它的代码就是好的.但是如果你用`Promise.resolve()`包裹,那么你总是可以确定稍后在`catch()`内会捕获它.
...
```
为什么这个可以工作？因为promise工厂在被调用前不会创造promise.它和一个then函数一样工作 - 事实上,它是一样的！

如果你看看`executeSequentially()`上面的功能,然后想象`myPromiseFactory`代替`result.then(...)`里面的.希望你会得到启发.
...
```
...至少,暂时可以这样.如果缩进成为一个问题,那么你可以将该函数提取到一个命名函数中：
```javascript
...
这实际上回到了前面关于promise和promise工厂的观点.总之,你可以直接传递一个promise给`then()`方法,但是它不会像你想的那样工作.`then()`的参数应该是一个函数,所以你的意思可能是：
```javascript
...
虽然优于回调,但promise仍然难以理解且容易出错.新手和专家都会经常把这些东西搞砸,实际上,这不是他们的错.问题是,promise虽然类似于我们在同步代码中使用的模式,但它不完全相同,而是一个合适的替代品.
